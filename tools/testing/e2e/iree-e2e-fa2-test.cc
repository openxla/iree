    ...
    for (int i = 0; i < seqLen_size; ++i) {
        for (int j = 0; j < seqLen_size; ++j) {
            ...
            for (int k = 0; k < headDim_size; ++k) {
                scores[i][j] += query_data[i * headDim_size + k] * key_data[j * headDim_size + k];
            }
    ...
    for (int i = 0; i < seqLen_size; ++i) {
        for (int j = 0; j < headDim_size; ++j) {
            ...
            for (int k = 0; k < seqLen_size; ++k) {
                result_data[i * headDim_size + j] += attention[i][k] * value_data[k * headDim_size + j];
            }
        }
    }
}
...
static iree_status_t reference_attention_element(
    iree_hal_dim_t batch_size, 
    iree_hal_dim_t numHeads_size, 
    iree_hal_dim_t seqLen_size,
    iree_hal_dim_t headDim_size,
...
// Reference attention implementation, used to compare attention results against.
static iree_status_t reference_attention(
  ...
  IREE_TRACE_ZONE_APPEND_VALUE_I64(z0, batch_size);
  ...
  for (iree_hal_dim_t b = 0; b < batch_size; ++b) {
...
// // Helper for check_attention_results: handler for the failure case.
// // If |file| is not NULL, detailed logging is written to it.
// static iree_status_t check_attention_failure(
...
static iree_status_t check_attention_results_impl(FILE* file,
                                            const attention_results_t* results,
                                            int check_every) {
  IREE_TRACE_ZONE_BEGIN(z0);
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, reference_attention(
              ...
              results->seqLen, 
              results->headDim,
              results->query_elem_type, 
              results->key_elem_type, 
              results->value_elem_type,
...
// Given an actual attention's inputs and output (all host-local), uses a reference
// attention implementation on the same inputs to check if the output is correct.
// On error, detailed logging is written to |file| if it is not NULL.
static iree_status_t check_attention_results(FILE* file,
                      const attention_results_t* results) {
  IREE_TRACE_ZONE_BEGIN(z0);
  // TODO: Increase the check every param to reduce the number of comparisons.
  int check_every = 1;
  iree_status_t status = check_attention_results_impl(file, results, check_every);
...
static const vm::NativeFunction<AttentionTestModuleState>
    kAttentionTestModuleFunctions[] = {
        vm::MakeNativeFunction("generate_fa2_components",
                               &AttentionTestModuleState::GenerateRandom4dTensor),
        vm::MakeNativeFunction("check_attention_results",
                               &AttentionTestModuleState::CheckAttentionResults),
};

struct AttentionTestModule final : public vm::NativeModule<AttentionTestModuleState> {
...
static iree_status_t fa2_test_module_create(iree_vm_instance_t* instance,
  ...
  *out_module = NULL;
  auto module = std::make_unique<iree::AttentionTestModule>(
      ...
      iree::span<const iree::vm::NativeFunction<iree::AttentionTestModuleState>>(
