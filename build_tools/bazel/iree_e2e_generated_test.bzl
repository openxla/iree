...
"""Macros for defining tests that use the iree-e2e-{test_name}-test runner."""
...
def iree_e2e_test(
    ...
    Args:
        ...
        test_name: Name of the test (e.g., matmuls, conv2ds).
    ...
    iree_bytecode_module(
        name = name + "_%s_module" % test_name,
...
def iree_single_backend_e2e_test(
        name,
        test_name,
        generator,
        test_runner,
    ...
    Args:
        ...
        generator: Target to run to generate the source MLIR files.
            ...
            to generator_args:
            --output_{$test_name}_mlir=(current binary dir)/name_{$test_name}.mlir
    ...
    native.genrule(
        ...
        cmd = " ".join([
            ...
            "--output_$(name %s)_mlir=$(location %s)" % (test_name, tests_src),
