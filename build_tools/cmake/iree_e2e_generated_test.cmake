...
function(iree_e2e_test)
  ...
  cmake_parse_arguments(
    _RULE
    ""
    "NAME;TEST_NAME;VARIANT_NAME;TESTS_SRC;TESTS_VMFB;CALLS_SRC;CALLS_VMFB;TRACE;TARGET_BACKEND;DRIVER;TEST_RUNNER;TEST_DEFINED;TEST_DISABLED"
  ...
  if(NOT TARGET "${_NAME}_${_RULE_TEST_NAME}_module")
    iree_bytecode_module(
      NAME
        "${_RULE_NAME}_${_RULE_TEST_NAME}_module"
      MODULE_FILE_NAME
        "${_RULE_TESTS_VMFB}"
      SRC
        "${_RULE_TESTS_SRC}"
  ...
  add_dependencies(
    ...
    "${_NAME}_${_RULE_TEST_NAME}_module"
    "${_NAME}_calls_module"
    "${_RULE_TEST_RUNNER}"
  )
...
#         --output_${TEST_NAME}s_mlir=${CMAKE_CURRENT_BINARY_DIR}/name_${TEST_NAME}s.mlir
...
#   TARGET_CPU_FEATURES: If specified, a string passed as argument to
#       --iree-llvmcpu-target-cpu-features.
function(iree_single_backend_e2e_test)
  if(NOT IREE_BUILD_TESTS)
    return()
  endif()
  ...
  cmake_parse_arguments(
    _RULE
    ""
    "NAME;TEST_NAME;GENERATOR;TARGET_BACKEND;DRIVER;TEST_RUNNER"
  ...
  set(_TESTS_SRC "${CMAKE_CURRENT_BINARY_DIR}/${_RULE_NAME}_${_RULE_TEST_NAME}.mlir")
  set(_CALLS_SRC "${CMAKE_CURRENT_BINARY_DIR}/${_RULE_NAME}_calls.mlir")
  set(_TESTS_VMFB "${CMAKE_CURRENT_BINARY_DIR}/${_RULE_NAME}_${_RULE_TEST_NAME}.vmfb")
  set(_CALLS_VMFB "${CMAKE_CURRENT_BINARY_DIR}/${_RULE_NAME}_calls.vmfb")

  list(APPEND _GENERATOR_STANDARD_FLAGS "--output_${_RULE_TEST_NAME}_mlir=${_TESTS_SRC}")
  ...
  if(NOT _RULE_TARGET_BACKEND STREQUAL "llvm-cpu" OR NOT IREE_ENABLE_TSAN)
    ...
    # iree_e2e_test is _NAME, even though we passed _RULE_NAME to it,
...
# iree_generated_e2e_test()
#
# Creates a set of iree_single_backend_e2e_test's differing
...
function(iree_generated_e2e_test)
  if(NOT IREE_BUILD_TESTS)
    return()
  endif()
  ...
  cmake_parse_arguments(
    _RULE
    ""
    "NAME;TEST_NAME;GENERATOR;TEST_RUNNER"
