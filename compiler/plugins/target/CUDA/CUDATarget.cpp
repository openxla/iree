...
struct CUDAOptions {
  ...
  void bindOptions(OptionsBinder &binder) {
    ...
    binder.opt<std::string>("iree-cuda-target", clTarget,
                            llvm::cl::cat(category),
                            llvm::cl::desc("CUDA LLVM target."));

    binder.opt<std::string>(
        "iree-cuda-target-features", clTargetFeatures, llvm::cl::cat(category),
        llvm::cl::desc(
            "CUDA target features; e.g., use '+ptxNN to set PTX version."));

    binder.opt<bool>(
        "iree-hal-cuda-use-ptxas", clUsePtxas, llvm::cl::cat(category),
        llvm::cl::desc(
            "It uses the ptxas compiler that is on the environment, compiles "
            "the generated PTX code with it, puts the cubin binary generated "
            "by ptxas into the executable. '--iree-cuda-target' is used as the "
    ...
    binder.opt<std::string>(
        ...
        llvm::cl::desc(
            ...
            "executable. '--iree-cuda-target' is used as the target GPU. If "
            "passing additional parameters to ptxas is desired, the parameters "
            "flag can be used (e.g.'--iree-hal-cuda-use-ptxas-params=-v')."));
